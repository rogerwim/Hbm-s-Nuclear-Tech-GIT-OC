...
public class TileEntityRBMKConsole extends TileEntityMachineBase implements IControlReceiver, IGUIProvider, SimpleComponent, ManagedPeripheral {
	...
	public String getComponentName() {
		return CompatHandler.Compats.RBMK_CONSOLE.name;
	}
	...
	public Object[] invoke(String method, Context context, Arguments args) throws Exception {
		switch (method) {
			...
			case ("setLevel"):
				...
				for (int i_1 = -7; i_1 <= 7; i_1++) {
					for (int j = -7; j <= 7; j++) {
						TileEntity te_1 = worldObj.getTileEntity(targetX + i_1, targetY, targetZ + j);

						if (te_1 instanceof TileEntityRBMKControlManual) {
							TileEntityRBMKControlManual rod = (TileEntityRBMKControlManual) te_1;
							rod.startingLevel = rod.level;
			...
			case ("setColumnLevel"):
				int x_1 = args.checkInteger(0) - 7;
				int y_1 = -args.checkInteger(1) + 7;
				double new_level_1 = args.checkDouble(2);

				TileEntity te_1 = worldObj.getTileEntity(targetX + x_1, targetY, targetZ + y_1);

				if (te_1 instanceof TileEntityRBMKControlManual) {
					TileEntityRBMKControlManual rod = (TileEntityRBMKControlManual) te_1;
					rod.startingLevel = rod.level;
					new_level_1 = Math.min(1, Math.max(0, new_level_1));

					rod.setTarget(new_level_1);
					te_1.markDirty();
					return new Object[]{};
				}
				return new Object[]{"No control rod found at " + (x_1 + 7) + "," + (7 - y_1)};
			case ("setColorLevel"):
				int color = args.checkInteger(0);
				double new_level_2 = args.checkDouble(1);
				boolean foundRods_1 = false;
				if (color >= 0 && color <= 4) {
					for (int i_1 = -7; i_1 <= 7; i_1++) {
						for (int j = -7; j <= 7; j++) {
							TileEntity te_2 = worldObj.getTileEntity(targetX + i_1, targetY, targetZ + j);

							if (te_2 instanceof TileEntityRBMKControlManual) {
								TileEntityRBMKControlManual rod = (TileEntityRBMKControlManual) te_2;
								if (rod.color == RBMKColor.values()[color]) {
									rod.startingLevel = rod.level;
									new_level_2 = Math.min(1, Math.max(0, new_level_2));

									rod.setTarget(new_level_2);
									te_2.markDirty();
									foundRods_1 = true;
								}
							}
			...
			case ("setColor"):
				int x_2 = args.checkInteger(0) - 7;
				int y_2 = -args.checkInteger(1) + 7;
				int new_color = args.checkInteger(2);
				if (new_color >= 0 && new_color <= 4) {
					TileEntity te_2 = worldObj.getTileEntity(targetX + x_2, targetY, targetZ + y_2);
					...
					return new Object[]{"No control rod found at " + (x_2 + 7) + "," + (7 - y_2)};
				}
