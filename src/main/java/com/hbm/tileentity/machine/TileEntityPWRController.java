...
public class TileEntityPWRController extends TileEntityMachineBase implements IGUIProvider, IControlReceiver, SimpleComponent, IFluidStandardTransceiver {
	...
	public double rodLevel = 100F;
	public double rodTarget = 100F;
		...
		if(!worldObj.isRemote) {
			...
			if(this.assembled) {
				...
				if(this.unloadDelay <= 0) {
					...
					if(this.rodTarget < this.rodLevel) this.rodLevel--;
					if(Math.abs(this.rodTarget - this.rodLevel) < 1) this.rodLevel = this.rodTarget;
					...
					if(this.coreHeat > this.coreHeatCapacity && !GameruleHandler.getPWRMeltdownDisabled(worldObj)) {
						meltDown();
					}
				} else {
					this.hullHeat = 0;
	...
	public void networkUnpack(NBTTagCompound nbt) {
		...
		rodTarget = nbt.getDouble("rodTarget");
